...
"""
Django Views for Multi-Agent RAG System
Updated to use LangGraph GraphExecutor instead of MultiAgentCoordinator.

Changes from previous version:
  - Removed: _coordinator singleton, get_coordinator() function
  - Removed: MultiAgentCoordinator import
  - query_rag       → uses get_executor().execute()
  - health_check    → checks executor/graph instead of coordinator
  - agent_status    → returns graph node list from executor
  - rl_feedback     → uses RLDecisionAgent directly (no coordinator needed)
  - rl_stats        → uses RLDecisionAgent directly
  - rl_train        → uses RLDecisionAgent directly
...
from apps.rag.models import (
    Document, Query, Session, AgentExecution,
    RLExperienceRecord, RLEpisodeSummary,
)
from apps.rag.serializers import (
    QueryRequestSerializer,
    DocumentListSerializer,
...
from apps.rag.filters import DocumentFilter, QueryFilter
from apps.rag.services.core_services import (
    get_llm_service,
    get_embedding_service,
    get_vector_store,
)
from apps.rag.services.graph.executor import get_executor          # ← NEW
from apps.rag.services.document_processor import DocumentProcessor
...
# ─────────────────────────────────────────────────────────────────────────────
#  RL AGENT HELPER  (replaces coordinator.rl_agent throughout)
# ─────────────────────────────────────────────────────────────────────────────

def _get_rl_agent():
    """
    Return the process-level RLDecisionAgent singleton.
    Previously accessed via coordinator.rl_agent — now accessed directly
    because coordinator.py has been removed.
    """
...
def _save_rl_episode(result: dict, query_record) -> None:
    ...
    Persist a per-query episode summary.
    Called at the end of query_rag(). Safe when RL metadata is absent.
    GraphExecutor already calls this internally, but we keep it here
    as a safety net in case the executor skips it on an error path.
    """
    rl_meta = result.get("rl_metadata", {})
    if not rl_meta:
        return
...
def query_rag(request):
    ...
    try:
        ...
        #   OLD: result = asyncio.run(coordinator.execute(query_text, context))
        #   NEW: two lines below replace the entire get_coordinator() pattern
        executor = get_executor()
        result   = asyncio.run(executor.execute(query_text, context))
        ...
        # Safety-net RL episode persist (executor already does this internally)
        _save_rl_episode(result, query_record)
        ...
        return Response({
            ...
            'query_type':        result.get('query_type'),
            'rl_metadata':       result.get('rl_metadata', {}),
            'evaluation_result': result.get('evaluation_result', {}),   # ← NEW field
        }, status=status.HTTP_200_OK)
...
def health_check(request):
    """
    Health check endpoint.
    Previously checked 'coordinator' — now checks 'graph' (LangGraph executor).
    """
    components  = {
        k: 'unknown'
        for k in ['database', 'llm_service', 'embedding_service', 'vector_store', 'graph']
    }
    all_healthy = True
    ...
    checks = [
        ...
        ('embedding_service', get_embedding_service),
        ('vector_store',      lambda: get_vector_store().get_count()),
        # 'coordinator' → 'graph': verifies LangGraph pipeline compiles correctly
        ('graph',             lambda: get_executor().graph),
    ]
    ...
    return Response({
        'status':     'healthy' if all_healthy else 'degraded',
        'timestamp':  timezone.now(),
        'version':    '2.0.0',           # bumped: now LangGraph-powered
        'components': components,
    })
...
def agent_status(request):
    """
    Get pipeline status.
    Previously returned coordinator.get_agent_status().
    Now returns LangGraph node list + RL stats.
    """
    try:
        executor = get_executor()
        ...
        return Response({
            'pipeline':     'langgraph_multiagent_v3',
            'graph_nodes':  [
                'planner_node',
                'retriever_node',
                'rl_decision_node',
            ...
            'rl_stats':     rl_agent.get_rl_stats(),
            'rl_enabled':   True,
            'dqn_enabled':  True,
            'crew_enabled': True,
            'langgraph':    True,
...
# ─────────────────────────────────────────────────────────────────────────────
#  QUERY HISTORY & DEBUGGING  (unchanged)
# ─────────────────────────────────────────────────────────────────────────────
...
def list_queries(request):
    ...
    limit     = int(request.GET.get('limit', 20))
    queryset  = Query.objects.all().order_by('-created_at')
    filterset = QueryFilter(request.GET, queryset=queryset)
    queries   = filterset.qs[:limit]
    return Response({
        'total':   Query.objects.count(),
        'showing': len(queries),
...
def get_query_execution(request, query_id):
    ...
    try:
        ...
        return Response({
            ...
            'document_filter':   query.metadata.get('document_filter'),
            'rl_metadata':       query.metadata.get('rl_metadata', {}),
            'evaluation_result': query.metadata.get('evaluation_result', {}),
        })
    except Query.DoesNotExist:
        return Response({'error': 'Query not found'}, status=status.HTTP_404_NOT_FOUND)
...
def rl_feedback(request):
    """
    Submit user feedback for a completed query.
    Triggers a deferred reward update in the DQN.

    POST /api/rag/v1/rl/feedback/
    Body: { "query_id": "...", "feedback": "positive" | "negative" }

    Change: previously used coordinator.rl_agent — now uses _get_rl_agent() directly.
    """
    serializer = UserFeedbackSerializer(data=request.data)
    if not serializer.is_valid():
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    try:
        rl_agent = _get_rl_agent()                          # ← changed
        rl_agent.apply_user_feedback(query_id, feedback)   # ← same method
        ...
        return Response(
            {
                ...
                "query_id": query_id,
                "feedback": feedback,
                "message":  "DQN updated with user feedback",
            },
            status=status.HTTP_200_OK,
        )
    except Exception as exc:
...
def rl_stats(request):
    ...
    try:
        rl_agent   = _get_rl_agent()                        # ← changed
        live_stats = rl_agent.get_rl_stats()                # ← same method
...
def rl_train(request):
    ...
    try:
        rl_agent     = _get_rl_agent()                          # ← changed
        replay_result = rl_agent.run_replay(batch_size=batch_size)  # ← same method
